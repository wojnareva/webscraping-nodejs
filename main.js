...
axios.get(url)  // Send HTTP GET request to the target page
    .then((response) => {
        ...
        $('.product-list__item').each(function () {
            ...
            const imageSource = item.find('img').attr('data-src') ||
                    item.find('source[type="image/jpeg"]').attr('srcset') ||
                    '';
            const imageUrl = imageSource.replace('100x100', '440x440')
            .replace('.webp', '.jpg');;
            ...
            if (title) {
                ...
                results.push({ author, title, state, priceText, price, ratingText, rating, imageSource, image: imageUrl, link });
            }
        });
